...
const Clothing = class Clothing {
  ...
  static create(req, res) {
    return db.execute(
      'INSERT INTO clothing_xx (id, name, cat_id, price, remote_url, local_url) VALUES (?, ?, ?, ?, ?, ?)',
      [
        req.body.id,
        req.body.name,
        req.body.cat_id,
  ...
  static updateById(req, res) {
    ...
    return db.execute(
      'UPDATE clothing_xx SET name = ?, cat_id = ?, price = ?, remote_url = ?, local_url = ? WHERE id = ?',
  ...
  static deleteById(id) {
    return db.execute('DELETE FROM clothing_xx WHERE id = ?', [id]);
  }
};
